...
assert_output() {
  ...
  local pattern="$3"
  local error_msg="$4"
  if ! echo "$output" | grep "$grep_flag" "$pattern"; then
    echo "${error_msg} Output was: $output"
    return 1
  fi
}
...
assert_output "$OUTPUT1" -q '"decision": "continue"' "Test 1 Failed!" || exit 1
...
assert_output "$OUTPUT2" -qv '"decision": "continue"' "Test 2 Failed (Prefix false positive)!" || exit 1
...
assert_output "$OUTPUT3" -qv '"decision": "deny"' "Test 3 Failed (Blocked non-commit command)!" || exit 1
...
assert_output "$OUTPUT4" -q '"decision": "deny"' "Test 4 Failed!" || exit 1
