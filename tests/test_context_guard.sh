...
assert_output() {
  ...
  echo "$output" | grep "$grep_flag" "$pattern" >/dev/null || { echo "$error_msg"; exit 1; }
}
...
assert_output "${OUTPUT1}" "-q" '"decision": "continue"' "Test 1 Failed! Output was: $OUTPUT1"
...
assert_output "${OUTPUT2}" "-qv" '"decision": "continue"' "Test 2 Failed (Prefix false positive)! Output was: $OUTPUT2"
...
assert_output "${OUTPUT3}" "-qv" '"decision": "deny"' "Test 3 Failed (Blocked non-commit command)!"
...
assert_output "${OUTPUT4}" "-q" '"decision": "deny"' "Test 4 Failed! Output was: $OUTPUT4"
...
assert_output "${OUTPUT5}" "-q" '"decision": "allow"' "Test 5 Failed! Output was: $OUTPUT5"
...
[ "$OUTPUT6" = "{}" ] || { echo "Test 6 Failed! Output was: $OUTPUT6"; exit 1; }
rm -f src/api/handler.js docs/context/api.md
...
assert_output "${OUTPUT7}" "-q" '"decision": "continue"' "Test 7 Failed! Output was: $OUTPUT7"
echo "Scenario 7 Passed: Fallback to AGENTS.md works."

echo "Scenario 8: Priority of GEMINI.md over AGENTS.md"
...
assert_output "${OUTPUT8}" "-q" '"decision": "continue"' "Test 8 Failed! Priority not respected. Output was: $OUTPUT8"

rm src/api_v3/handler.js
touch src/api/handler.js
OUTPUT9=$(./scripts/context-guard.sh --check-stop)
assert_output "${OUTPUT9}" "-qv" '"decision": "continue"' "Test 9 Failed! AGENTS.md was read instead of GEMINI.md. Output was: $OUTPUT9"
echo "Scenario 8 Passed: GEMINI.md prioritized over AGENTS.md."

echo "Scenario 9: Priority of AGENTS.md over .agents/AGENTS.md"
git rm -f GEMINI.md -q
mkdir -p .agents
...
mkdir -p src/api_v4
# We already have AGENTS.md from Scenario 7 pointing to /src/api/
...
assert_output "${OUTPUT10}" "-q" '"decision": "continue"' "Test 10 Failed! Priority of AGENTS.md not respected. Output was: $OUTPUT10"

rm src/api/handler.js
touch src/api_v4/handler.js
OUTPUT11=$(./scripts/context-guard.sh --check-stop)
assert_output "${OUTPUT11}" "-qv" '"decision": "continue"' "Test 11 Failed! .agents/AGENTS.md was read instead of AGENTS.md. Output was: $OUTPUT11"
echo "Scenario 9 Passed: AGENTS.md prioritized over .agents/AGENTS.md."

echo "Scenario 10: Fallback to .agents/AGENTS.md"
git rm -f AGENTS.md -q
git commit -m "remove AGENTS.md" -q

# .agents/AGENTS.md pointing to /src/api_v4/ is active
touch src/api_v4/handler.js
OUTPUT12=$(./scripts/context-guard.sh --check-stop)
assert_output "${OUTPUT12}" "-q" '"decision": "continue"' "Test 12 Failed! Fallback to .agents/AGENTS.md not respected. Output was: $OUTPUT12"
