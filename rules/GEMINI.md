...
2. **The Specification Gate (Phase 3)**:
   - Must be authored to `docs/specs/YYYY-MM-DD-[feature]-spec.md` with RFC 2119 language, quantitative SLAs, and runnable schemas. You MUST NOT start planning until the user explicitly approves the spec.
3. **The Implementation Plan Gate (Phase 4 & 5)**:
   - Must be authored to `docs/plans/YYYY-MM-DD-[feature]-plan.md` with bite-sized tasks (2–5 min), copy-pasteable code blocks, and zero placeholders (`TODO`/`TBD`). You MUST NOT code until the plan passes adversarial audit.
...
5. **The Living Docs & ADR Gate (Phase 9)**:
   - Any architectural changes MUST be recorded in `docs/adr/NNNN-[title].md` and indexed in `docs/adr/README.md` before release closure.
