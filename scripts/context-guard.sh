...
if [ "$MODE" = "--check-commit" ]; then
  ...
  IS_COMMIT="false"
  if [ -n "$PAYLOAD" ]; then
    IS_COMMIT=$(echo "$PAYLOAD" | jq -r 'try (.toolCall.args | (.CommandLine // .commandLine // "") | if test("git commit|git-commit") then "true" else "false" end) catch "false"' 2>/dev/null || echo "false")
